...
- beware: JS has block scope-like syntax using curly braces, but `var` is not block scoped, e.g. `i` in `for (var i = 0; ...) {...}` is a global variable, keeps existing outside in global scope ⚠️ prefer `let` and `const` at all times 
...
- unpack values from arrays and object properties into distinct variables
