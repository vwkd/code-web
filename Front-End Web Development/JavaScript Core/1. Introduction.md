...
- template literal: back-ticks instead of quotes, e.g. `&grave;Hello World!&grave;`, can run over multiple lines and do string interpolation using `${expression}`, e.g. `${myStr.includes('cool') ? 'hay' : 'nay'}`
...
- any matching case gets executed on after the other, needs to prevent with `break` keyword in each
...
- doesn't necessarily iterate in order if some properties were removed❓
...
- use `&&` to check for null objects during assignment, e.g. `let name = obj && obj.name` evaluates to null when `obj` doesn't exists since it's set to `obj ` which is null, else to `obj.name`
...
- deletes an object, object property or array item
- returns true upon successful deletion, else false
- sets deleted thing to undefined, i.e. doesn't shrink array❗️
...
- beware: arrays and null are confusingly both "object"
...
- `...`: spread operator, expands iterable in argument
