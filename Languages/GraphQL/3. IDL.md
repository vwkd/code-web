...
- beware: types nullable by default is opposite of most type systems like TypeScript, e.g. optional type `T?` for type `T`
- beware: for input type: `null` value and omission both describe lack of value, but separate things ❗️
...
- beware: `NamedType` must be an Object type, currently doesn't allow union of primitive types, see [#215](https://github.com/graphql/graphql-spec/issues/215) ⚠️
...
- beware: if uses a union as type for field, can query no fields without using fragment with type condition ❗️
- beware: differs from TypeScript where can access a property that is on all members of a union of object types ⚠️
...
- syntax: `[ T ]` for type `T`
- field of list type is usually named plural, has argument for first/last N items, e.g. `users(first: String!)`
- field of list type should implement pagination, see Pagination
...
- beware: don't confuse with Object type, doesn't allow field arguments or references to abstract types ❗️
