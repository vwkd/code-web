...
- boxes with same stack level are stacked by `order` property, or by order in document tree otherwise, last one is on top
- beware: in Flow Layout is always document tree order since `order` property only applies in Flow / Grid Layout ❗️
...
- beware: flex / grid items of same stack level are stacked by `order` property before resorting to document tree order ❗️
