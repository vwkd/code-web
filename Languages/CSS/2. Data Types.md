...
- beware: allows for 8 equivalent values for same position, 4 for different origins, times 2 for reversed order ❗️
...
may produce different data types depending on input
?? functions, e.g. `toggle()`
mathematical functions, e.g. `calc()`, `count()`, etc.
