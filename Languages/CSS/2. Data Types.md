...
- beware: data types used only by value of certain properties are defined where they're used, e.g. see Grid Layout for `<flex>` data type ❗️
...
- define pixel unit as linear size `S = 2*D*tan(0.0213 deg/2)`, function of viewing distance `D`  
...
- beware: physical unit of pixel unit varies with DPI for same `D`, as close to reference pixel as possible ❗️
...
- `<image>` < data type
`<gradient>` is special `<image>` data type
...
- beware: positive axis direction is inwards to positioning area, i.e. in negative x-/y-axis direction for `right` / `bottom` origin ❗️
