...
??- beware: flexible track sizing may have "eaten up" free space, except if grown / shrunk size hit limit of maximum / minimum size, or if distributes less than 100% of free space due to factors being smaller than one ❗️
...
??- beware: has no effect if grid container has `auto` block size since no free space ❗️
...
- grid rows grow to fill the container in the block dimension
- size is grown equally, not proportionally
?? BEWARE: RESIZES TRACKS !!!
...
??- beware: only has effect if free space in grid area, e.g. track size is not content-based ❗️
- not inherited
...
???? REALLY IN GRID AS WELL ???- beware: can cause item to shrink to fit container as well ❗️

<!-- todo: EXAMPLE DOESN'T SHOW SHRINKING AS EXPECTED -->
