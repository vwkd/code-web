...
- beware: size of a absolutely / fixedly positioned element is only changed when size is `auto` and both offset properties are non-`auto` ❗️
...
- nearest ancestor box that uses one of the properties with a non-initial value: `position` (only for absolute positioning), `transform`, `will-change`, `contain`, etc.
