...
- some shorthans allow for combination of shorthands, e.g. `border` combines `border-width`, `border-color`, `border-style`
'border' property cannot set different values on the four borders
- can be in any order ???
- at least one must be specified

beware: Omitted values are set to their initial values.
