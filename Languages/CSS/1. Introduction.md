...
- values have a specific type, e.g. string, number, color, etc., see Units
- if string value contains spaces needs quotation marks, e.g. `font-family: "Helvetica Neue"`
...
- elements have no default style, are all the same for CSS
- all different appearance set by user agent style sheet
- beware: always choose elements by semantic meaning, then style as desired, overwrite user agent default style, e.g. `<ul>` for navigation menu since it's a list
