...
- descendant elements can inherit properties from parent elements, saves lot of work, e.g. not setting color for every individual element in tree
- by default only some elements inherit some properties
- by default most elements inherit most text styling properties, e.g. `color`, `font-size`, etc., except `<a>` doesn't inherit `color`
- inheritance of a property is overwritten by setting it to a value
- inheritance of a property can be forced by setting it to a specific global value
  - `inherit`: inherit value from parent, e.g. `a {color: inherit}` takes text color from parent
  - `initial`: user agent default value, e.g. `a {color: initial}` is blue
  - `unset`: like `inherit` if property is usually inherited, like `initial` if not, e.g. `a {color: unset}` inherits, because `color` is inherited by default ???
- use inheritance for majority of elements, style exceptions manually
...
3. Sort declarations by selector specificity, apply only most specific
    - (for inline styles skip to 4. directly ???)
...
- user agent has default style designed for documents, e.g. `<h1>` is big and bold, `<ul>` has `list-style-type: disc`, etc.
- use a "reset" style sheet to undo any default style, e.g. [destyle.css](https://github.com/nicolas-cusan/destyle.css)
- use a "normalize" style sheet to unify default style accross different user agents, e.g. [normalize.css](https://github.com/necolas/normalize.css), [sanitize.css](https://github.com/csstools/sanitize.css), [cssremedy](https://github.com/jensimmons/cssremedy), [postcss-normalize](https://github.com/csstools/postcss-normalize), etc., see [html5-kitchen-sink](https://github.com/dbox/html5-kitchen-sink) for comparison
