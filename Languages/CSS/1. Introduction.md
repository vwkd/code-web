...
- a property has by default an initial value, defined by specification, i.e. no property remains "undefined"
- for _all elements_ for which no value is set and no one is inherited, the initial value is used, see Inheritance later
...
- properties of descendant elements can inherit values from their parent elements property, saves lot of work, e.g. not setting color for every individual element in tree
- by default a property can be inherited or not, defined by specification, e.g. most text styling properties are inherited, like `color`, `font-size`, etc.
- for _all elements_ for which no value is set for an inherited property the inherited value is used instead of the initial value, only the root element uses the initial value because it has no parent
...
- beware: an inherited property set by user agent style sheet makes it appear as if didn't inherit for that specific element, but property does inherit, just user agent set a value like any other style sheet, e.g. `color` of anchor element (with `href` attribute) is blue ⚠️
- use inheritance to style majority of elements, style exceptions manually
...
- `initial`: initial value, beware: not "default value" from user agent style sheet, i.e. as if all declarations of the property for _all_ elements in style sheets of any origin were erased
- `unset`: like `inherit` if property inherits, otherwise like `initial`, i.e. as if all declarations of the property for _selected_ element(s) in style sheets of any origin were erased
- `revert`: like `unset`, except limited to same style sheet origin (user agent, user, author), earlier style sheets can still apply (see Cascade), i.e. as if all declarations of the property for _selected_ element(s) in style sheets of same origin were erased
...
- styles are assigned according to the cascade

### The cascade

<!-- ToDo: Sources? Inline style? -->

- algorithm to determine style of an element

1. Find all rules for element, apply all declarations
    - can come from different style sheets or different selectors
    - if for one property there are multiple declarations, go to 2.
2. Sort declarations by origin and importance, apply only most specific
...
3. Sort declarations by selector specificity, apply only most specific
...
4. Sort declarations by rule appearance, apply only last defined

- all rules are applied, only for conflicting declarations it uses the one from the most specific rule
- order of rules and declarations doesn't matter, except when selectors have the same specificity
- user stylesheet with `!important` wins, because accessibility requires custom style 
- order doesn't matter for specificity selector, just quantity of selectors, e.g.  
  `body p` ≙ `main p`, `.myclass#myid` ≙ `#myid.myclass`
- can artificially increase specificity by chaining same ID or class, e.g.  
