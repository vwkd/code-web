...
- determines bounding grid lines of grid area automatically if not (fully) defined in manual placement
...
- values: `auto`, `normal`, `start`, `end`, `center`
- initial value: `auto`
- beware: no `stretch` value since this is ??? ❗️
...
???beware: no `stretch` value since ??? ❗️
...
sizes the tracks instead of the items themselves
