...
- shifting of grid item within free space of its grid area and grid lines within free space of grid container
...
??? by default item in cell is stretched to fill
??? "justify" is for row axis, "align" is for column axis, doesn't change with writing mode unlike in Flex Layout !!!

### Inline axis

- specifies alignment of grid item in grid area along inline axis
- `justify-self`
- not inherited
- applies to grid items
- values: `auto`, `normal`, `start`, `end`, `center`, `stretch`
- initial value: `auto`

- `justify-items` is shorthand that sets `justify-self` of all children
- not inherited
- applies to grid containers
- values: `auto`, `normal`, `start`, `end`, `center`, `stretch`
- initial value: `start` ??? DEFAULT IS STRETCH, STRETCHES ITEM OVER WHOLE GRID CELL IN ROW DIRECTION
...
- `align-items` is shorthand that sets `align-self` of all children
- not inherited
- applies to grid containers
- values: `auto`, `normal`, `start`, `end`, `center`, `stretch`
- initial value: `start` ??? DEFAULT IS STRETCH, STRETCHES ITEM OVER WHOLE GRID CELL IN COLUMN DIRECTION

### Inline axis track

- specifies alignment of grid columns in grid container
- beware: applies to all columns since can't target individual tracks, use self-alignment properties instead ❗️
- `justify-content`
- not inherited
- applies to grid containers
- values: `start`, `end`, `center`, `space-between`, `space-around` ???`space-evenly`
- initial value: `start`
??? NO, THERE IS, BUT IT DOESN'T DO ANYTHING IN FIREFOX beware: no `stretch` value since ??? ❗️
???beware: `stretch` resizes track size ?!?!?!?! only minmax or everything

??? beware: has no effect if flex container has `auto` block size since no free space ❗️
...
- specifies alignment of grid rows in grid container
- beware: applies to all rows since can't target individual tracks, use self-alignment properties instead ❗️
- `align-content`
- not inherited
- applies to grid containers
- values: `start`, `end`, `center`, `space-between`, `space-around` ???`space-evenly`
...
<!-- ToDo: integrate in Alignment, unite with Flex version -->
...
- two dimensional alignment due to `justify-self`
