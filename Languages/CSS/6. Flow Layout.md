...
- default document layout, invented for documents
...
- beware: use physical terminology here, assume default writing mode, for logical terminology see Writing Mode, e.g. vertical ≙ block direction, horizontal ≙ inline direction, width / height ≙ block / inline size, etc.
...
- beware: vertical padding and border properties don't increase line height, overlap into adjacent lines, e.g. single `<a>` in parent isn't contained vertically ⚠️
<!-- so what is use case of vertical margin in inline layout ??? -->
...
- beware: don't confuse with anonymous wrapper box for text in element that has other child elements ❗️
...
- margins are contained in parent, see Margin Collapse 🎉
...
- `inline` is default, except for `ruby` IDT `block` is default
...
- in-flow / out-of-flow box: purely visual characteristic if a box follows Flow Layout, confusing since not uniform for irregular layouts, e.g. relatively positioned elements are in-flow while floats and absolutely positioned elements are out-of-flow, also doesn't make sense for other regular layouts at all, e.g. Grid Layout
