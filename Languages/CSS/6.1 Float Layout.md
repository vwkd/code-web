...
- beware: don't use for page layout, instead use modern Flex or Grid Layout, only use if needs to *flow around* something, e.g. text around image, etc. ❗️
- interacts with in-flow boxes across formatting contexts, e.g. following siblings of ancestors or descendants of following siblings
...
- size is `max-content` by default
...
- values: `none` (default), `left` / `right` (physical), `inline-start` / `inline-end` (logical, see Writing Mode)
- not inherited
...
- beware: only needed on single element, since following ones adjust to it due to being in-flow ❗️
...
- doesn't grow to contain floats, size is determined by in-flow children, zero if only float children, makes float bleed out of block-end of parent if too tall
...
- legacy hack 1: add empty in-flow block-level box to clear float, but pollutes markup
...
- legacy hack 4: make wrapper itself a float or absolutely positioned, but has side effects of activating the specific layout, also needs explaining comment
