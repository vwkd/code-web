...
- iterator is closed automatically if terminates early anywhere, e.g. `break`, `return`, `throw` (uncaught) in `for...of` loop, etc.
- can make iterator iterable with `Symbol.iterator` method returning itself
...
- beware: values for which `done` is `true` don't show up in the iteration sequence, e.g. `.return()`, etc. ❗️
...
- in ES6 method shorthand within object literal or class the `*` goes before the name
