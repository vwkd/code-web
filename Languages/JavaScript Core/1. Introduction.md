...
<!-- ToDo: periodically remove ES version notes, i.e. <= ES6 -->
...
- just-in-time compiled object oriented programming language  
  like other languages two passes, first parsing, then execution
...
- when object destructuring into already existing variables need to wrap in parentheses to not be interpreted as block
