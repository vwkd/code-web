...
- pages are web pages, exist as web pages on server
...
- pages are views, don't exist as web pages on server
...
- disadvantages:
  ...
  - dependent, because necessarily needs JavaScript besides HTML and CSS
- website can only be client-side rendered since pages are built on client
- usually web pages built with client-side component frameworks and a routing library, e.g. Svelte + Routify, React + React Router, etc.
- website can use server-side rendering for initial page, and client-side rendering for subsequent pages
- beware: needs to store static mirror of pages on server such that any page can be initial page
- usually web pages built with client-side component frameworks and additional framework, e.g. Svelte + Sapper, React + Gatsby, etc.
...
- manages underlying platform, e.g. operating system, web server, database, etc.
- server-side processes are bundled on single server
...
- doesn't manage underlying platform, instead uses Cloud services, e.g. static Web server, database, authentication, etc.
- server-side processes can be split into microservices, e.g. FaaS
