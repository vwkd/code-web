...
- independent of consumer, ready for future consumer
- "when in doubt leave it out"
- implement what you need, not what you think you need
...
returns fixed data
uses multiple endpoints, according to consumer needs, uses different HTTP verbs
request can not specify exactly what data it needs, each single endpoint sends too much data while doesn’t include all data needed, for nested data has n+1 round trips

advantages
  - HTTP cacheable, because HTTP response from endpoint is static (at least for the time that data stays up-to-date)
  - simple, because no additional query processing on server
drawbacks
  - high bandwith, because transmits more data than needs
  - high latency, because needs follow-up request for nested data
  - bad analytics, because doesn't know exactly what data consumers use
  - imperative, because has multiple endpoints
  - consumer-dependent, because API is dependent on subset of data that consumer chooses
  - not evolvable, because needs new endpoints, can't change response for a given endpoint


## GraphQL

JSON-like API query language
intended to be used with JSON

returns variable data, depending on GraphQL query
single endpoint, since data is variable, doesn't use different HTTP verbs, HTTP verb is POST
...
advantages
  - low bandwith, because just transmits data that needs
  - low latency, because no follow-up request for nested data
  - good analytics, because knows exactly what data consumers use
  - declarative, because has single endpoint
  - consumer-independent, because API is independent on subset of data that consumer chooses
  - evolvable, because doesn't need new endpoints, can add new fields without impacting existing consumers SINCE CONSUMERS SPECIFY WHAT WANT
drawbacks
  - not HTTP cacheable, because HTTP response from endpoint is variable
