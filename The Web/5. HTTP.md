...
- designed to transmit hypertext documents on the Web, but can transmit any file, today replaces SSH, FTP, etc.
- communication consists of independent request-response pairs, always initiated by client, i.e. pull-based communication
...
- transport layer packet is sent to port given in URL, by default port 80 for HTTP and port 442 for HTTPS
...
- Browser sends many requests when loading a single website, often "automatically" in background
...
- `HEAD` is like `GET`, but only response headers without response body, used to fetch metadata about a resource from the server without transmitting its content
- `TRACE` is used to see if intermediary servers modified the request
- most often needs to support only `GET` and `POST`, maybe `HEAD` ??
...
- beware: server may not follow HTTP standards, can do whatever it wants, e.g. may not treat POST/GET as un-/safe, may send wrong status codes, may not honor request headers like `Accept`, may not handle directory paths, may serve some random file back, etc. pp. ⚠️

### Format

- status code: number representing un-/successful response
...
- only on `type` of reponse, can distinguish between success and error using body and header, but response is same as always ???
...
- static server needs to only support GET, not POST
